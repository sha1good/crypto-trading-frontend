...
export const getUser = (token) => {
  return async (dispatch) => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/api/users/profiles`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });

      console.log("For get User " + response)
      const user = response.data;
...
export const sendVerificationOtp = ({ token, verificationType }) => {
  return async (dispatch) => {
...
export const enableTwoStepAuthentication = ({ token, otp }) => {
  //console.log("jwt", jwt);
  return async (dispatch) => {
